...
All research is stored at: `~/.config/opencode/learning/<topic-slug>/`
...
written as `scripts/<name>.sh`: in Claude Code the
...
- is safe to run several at once (no shared temp files) — and
  `scripts/fanout.sh` runs several of them in one call,
...
2. Check if `~/.config/opencode/learning/<topic-slug>/` exists
3. If not, create it: `mkdir -p ~/.config/opencode/learning/<topic-slug>/`
...
`scripts`. Every `*.sh` call has a `--help`.

### Learning material

| Material | Domain | First call | Then | Read it with |
|---|---|---|---|---|
| Papers | CS / ML | `scripts/arxiv.sh "<title or topic>" --max 5` (use `ti:"..."`, `au:`, `cat:cs.LG` for precision) | `scripts/s2.sh search "<topic>" --limit 5 --open-access` (citation counts, PDF links); `scripts/openalex.sh search "<topic>" --oa` | download `pdf_url`, then `Read` with `pages`; or `fetch-readable.sh <pdf_url>` |
| Papers | Medicine / biology | `scripts/pubmed.sh search '<topic> AND (systematic review[pt] OR meta-analysis[pt] OR randomized controlled trial[pt])' --max 8` | `openalex.sh search --type article`; `crossref.sh search` | `pubmed.sh abstract <pmid>` for the abstract; full text at the `pmc_url` PDF, then `Read` with `pages` |
| Papers | Any other field | `scripts/openalex.sh search "<topic>" --limit 8` | `scripts/crossref.sh search "<topic>" --type journal-article`; `s2.sh search` | `oa_url` / `pdf_url` → `Read` with `pages` |
...
| Articles / blogs / talks write-ups | any | built-in `WebSearch` (`"<concept>" site:<expert's domain>`, `"<concept>" "what actually matters"`) | `mcp__exa__web_search_exa` if present (describe the ideal page: "a practitioner's blog post explaining X from experience"); `scripts/websearch.sh` if Brave/Tavily keys are set | `scripts/fetch-readable.sh <url> --out <file>` then `Read` the file; `WebFetch` or `mcp__exa__web_fetch_exa` for JS-rendered pages |
...
| First-party evidence (the author's own words) | any | `WebSearch "<name>" site:<their domain>`; `scripts/github.sh repos "user:<handle>"` / `github.sh readme <owner/repo>` | `scripts/yt-search.sh "<name> <topic>" --channel "<name>"` (their own talks) | article → `fetch-readable.sh`; repo → clone; talk → `yt-transcript.sh` |
| Images / diagrams | any | `scripts/wikimedia-images.sh "<thing> diagram" --limit 8` | `scripts/openverse.sh "<thing>" --license by,by-sa,cc0` | `curl -sL <thumb_url> -o /tmp/<name>.png` then `Read` the file (you see the image); the paper's own figure via the PDF page |
| Videos / lectures | any | `scripts/yt-search.sh "<topic> lecture" --limit 8` | `yt-search.sh --channel "<university or expert>"`; `WebSearch "<topic>" site:youtube.com/playlist` | `scripts/yt-transcript.sh <id> --out <file>` then `Read` the whole file; `--timestamps` when you need to cite a moment |
...
| Code / repos | software | `scripts/github.sh repos "<topic> stars:>200" --sort stars` | `github.sh readme <owner/repo>`; `WebSearch "<topic>" site:github.com` | `git clone --depth 1 <url> /tmp/<name>` then `Read` the files that matter |
...
|---|---|---|---|---|
| Company facts (incorporation, officers, accounts) | UK | `scripts/companies-house.sh search "<name>"` → `companies-house.sh company <number>` → `companies-house.sh filings <number> --category accounts` | — | the filing PDF (`document_url`) via `Read`; keyless: the filing-history page via `fetch-readable.sh` |
| Company facts | US | `scripts/edgar.sh search '"<name>"' --limit 10` (any filer, incl. private companies via Form D) → `edgar.sh filings <ticker> --forms 10-K` → `edgar.sh facts <ticker> --concept <Tag>` | — | `fetch-readable.sh <url>` on the filing document (SEC needs `EDGAR_USER_AGENT`) |
...
| Funding / investors | US | `edgar.sh search '"<name>"' --forms D` (Form D: amount sold, date, investor count) | company press release: `WebSearch "<name>" raised site:<their domain>`; `scripts/news.sh "<name> raises" --when 1y` | the Form D XML / the press release page |
...
| Customer sentiment | any | `scripts/hn.sh "<product>" --comments --limit 30 --since <date>`; `hn.sh item <story id>` for a whole launch thread | `scripts/reddit.sh "<product> alternative" --subreddit <sub>` → `reddit.sh thread <url>`; G2/Capterra via `WebFetch` (bot-walled for curl) | quote verbatim with date and thread URL; note how many comments you read |
| Market size / macro data | UK | `scripts/ons.sh "<series>" --type dataset` → `fetch-readable.sh <url>` and download the CSV | `worldbank.sh indicator <CODE> --country GB` | the dataset page; store the **series id / dataset name and release date** |
| Market size / macro data | US / global | `scripts/worldbank.sh search "<keyword>"` → `worldbank.sh indicator <CODE> --country GB,US --mrv 5`; `scripts/fred.sh series <ID>` | Eurostat / OECD (`ons.sh --help` shows the URL patterns); Wikidata SPARQL for entity facts | JSON values; cite body + series id + year |
...
| News / events | any | `scripts/news.sh "<company or market>" --when 1m` | `news.sh --gdelt` (direct URLs, rate-limited); `mcp__exa__web_search_advanced_exa` with `category: "news"` and `startPublishedDate` | `fetch-readable.sh` the outlet's own URL (Google News links are redirectors); store the publish date |
| Patents / IP | any | `scripts/patents.sh "<technology>" --assignee "<company>" --after 2020-01-01` | `patents.sh --lens` with `LENS_API_KEY` | `fetch-readable.sh https://patents.google.com/patent/<id>/en` |
...
Scripts dir: scripts   (absolute; this skill's SKILL.md is one level up)
...
`scripts/fanout.sh '<cmd>' '<cmd>' ...` (or one command
...
2. `Bash`: `scripts/fanout.sh 'arxiv.sh "hash table collision resolution" --max 5' 'openalex.sh search "open addressing versus separate chaining" --limit 5' 'hn.sh "hash table collisions" --stories --limit 10' 'wikimedia-images.sh "hash table diagram" --limit 5'` — technical (papers), expert (what practitioners argue about), figures
...
1. `Bash`: `scripts/fanout.sh 'fetch-readable.sh https://docs.python.org/3/faq/design.html --out "$LA_OUT/python-faq.txt"' 'fetch-readable.sh <practitioner post url> --out "$LA_OUT/post.txt"' 'curl -sL <pdf_url> -o "$LA_OUT/paper.pdf"' 's2.sh recommend arXiv:<id> --limit 3' 'curl -sL <thumb_url> -o "$LA_OUT/diagram.png"'`
...
|---|---|
| Paper | `scripts/s2.sh recommend arXiv:<id> --limit 3` (or `DOI:<doi>`); if rate-limited, `scripts/openalex.sh related <doi or W-id> --limit 3`; for the follow-on literature `s2.sh citations <id> --limit 5` |
| Web page (article, product, company) | `scripts/exa.sh similar <url> --limit 3` when `EXA_API_KEY` is set; otherwise `mcp__exa__web_search_exa` with the page's title as the query, or `WebSearch "<title>"` / `"<product> alternatives"` |
...
For each concept researched, create or update `~/.config/opencode/learning/<topic-slug>/<concept-slug>.md`:
...
Under `~/.config/opencode/learning/<topic-slug>/` — one entity per file, same citation
