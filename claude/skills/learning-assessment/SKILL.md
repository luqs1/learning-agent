...
Every run of the checklist appends one `gate.check` event to the session trace file the Learning agent created at session start (`~/.claude/learning/.traces/<topic-slug>/<session-timestamp>.jsonl`, or under `$LEARNING_KB_ROOT` when that variable is set). One Bash command, using the literal trace-file path:
